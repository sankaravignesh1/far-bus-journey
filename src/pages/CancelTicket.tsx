...
const CancelTicket = () => {
  ...
  const toggleSeatSelection = (passengerId: string) => {
    if (bookingDetails) {
      setBookingDetails({
        ...bookingDetails,
        passengers: bookingDetails.passengers.map(passenger => passenger.id === passengerId ? {
          ...passenger,
  ...
  const confirmCancellation = () => {
    ...
    // Mock API call
    setTimeout(() => {
      const selectedSeats = bookingDetails?.passengers.filter(p => p.selected).map(p => p.seatNumber).join(', ');
      toast({
  ...
  return <Layout>
      <div className="container-custom py-12">
        ...
        {!bookingDetails ? <div className="max-w-md mx-auto card">
          ...
          </div> : <div className="max-w-2xl mx-auto card">
            {showConfirmation ? <div className="text-center">
                ...
                <div className="bg-gray-100 p-4 mb-6 rounded-md">
                  ...
                  <ul>
                    {bookingDetails.passengers.filter(passenger => passenger.selected).map(passenger => <li key={passenger.id}>
                ...
                <div className="flex justify-center space-x-4">
                  ...
                  <Button variant="destructive" onClick={confirmCancellation} disabled={loading}>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full">
                    ...
                    <tbody>
                      {bookingDetails.passengers.map(passenger => <tr key={passenger.id} className="border-b">
                          <td className="px-4 py-2">
                            <input type="checkbox" checked={passenger.selected} onChange={() => toggleSeatSelection(passenger.id)} className="h-4 w-4" />
                          </td>
                ...
                <div className="mt-6 text-center">
                  <Button onClick={handleCancelTickets} variant="destructive" disabled={!bookingDetails.passengers.some(p => p.selected)}>
