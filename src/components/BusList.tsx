...
const BusList: React.FC<BusListProps> = ({ buses, journeyDate }) => {
  ...
  const filteredBuses = buses.filter(bus => {
    ...
    const typeMatch = 
      (filters.ac && bus.type.toLowerCase().includes('ac') && !bus.type.toLowerCase().includes('non')) ||
      (filters.nonAc && bus.type.toLowerCase().includes('non')) ||
      (!filters.ac && !filters.nonAc);
    
    // Apply Sleeper/Seater filter
    const categoryMatch = 
      (filters.sleeper && bus.type.toLowerCase().includes('sleeper')) ||
      (filters.seater && bus.type.toLowerCase().includes('seater')) || 
