              ...
              <div className="space-y-4">
                <div className="relative">
                  <div className="flex items-center border border-far-gray rounded-md bg-white overflow-hidden">
                    ...
                    <input type="text" placeholder="From" className="input-field border-0 flex-grow" value={fromCity} onChange={e => setFromCity(e.target.value)} onFocus={() => setFromFocused(true)} onBlur={() => setTimeout(() => setFromFocused(false), 200)} required />
                  </div>
                  {fromFocused && <div className="absolute z-10 mt-1 w-full bg-white border border-far-lightgray rounded-md shadow-lg max-h-60 overflow-y-auto">
                      {cities.filter(city => city.name.toLowerCase().includes(fromCity.toLowerCase())).map(city => <div key={city.id} className="px-4 py-2 hover:bg-far-cream cursor-pointer" onClick={() => {
                ...
                <div className="relative">
                  <div className="flex items-center border border-far-gray rounded-md bg-white overflow-hidden">
                    ...
                    <input type="text" placeholder="To" className="input-field border-0 flex-grow" value={toCity} onChange={e => setToCity(e.target.value)} onFocus={() => setToFocused(true)} onBlur={() => setTimeout(() => setToFocused(false), 200)} required />
                  </div>
                  {toFocused && <div className="absolute z-10 mt-1 w-full bg-white border border-far-lightgray rounded-md shadow-lg max-h-60 overflow-y-auto">
                      {cities.filter(city => city.name.toLowerCase().includes(toCity.toLowerCase())).map(city => <div key={city.id} className="px-4 py-2 hover:bg-far-cream cursor-pointer" onClick={() => {
                  setToCity(city.name);
                  setToFocused(false);
                }}>
                            {city.name}
